...
// Connect to the backend server (replace with your server URL if different)
const socket = io("https://75club.games");
    ...
    success: function (response) {
      ...
      const displayResultHandler = ({ status, amount, period, result }) => {
        ...
        if (status === STATUS_MAP.WIN) {
          ...
          $(".modal-popup__amount").text(`₹ ${Number(amount).toLocaleString('en-IN')}`);
