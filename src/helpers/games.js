...
export function getNthMinuteSinceDayStart() {
  ...
  return diff; // 1-minute intervals
}

export function generatePeriods(gameRepresentationId) {
...
export async function getNewPeriod (period) {
  try {
     
      const now = new Date();
      const datePart = now.getFullYear().toString() + 
                      (now.getMonth() + 1).toString().padStart(2, '0') + // Month is 0-indexed, so add 1
           now.getDate().toString().padStart(2, '0'); // Day of the month
          // Format the timePart by zero-padding the counter to 4 digits
          let preDatePart = period.toString().slice(0, 8);
          let getLastFourDigits = period.toString().slice(-4);

          let timeCounter;
          if(datePart != preDatePart){
              getLastFourDigits = "0000"
          }
          timeCounter = parseInt(getLastFourDigits) + 1;
          let timePart = timeCounter.toString().padStart(4, '0');

          // Combine datePart and timePart to form the full period
...
export function getBonuses(amount) {
  const bonusTable = {
    100: { selfBonus: 28, uplineBonus: 0 },
    200: { selfBonus: 48, uplineBonus: 0 },
    500: { selfBonus: 108, uplineBonus: 0 },
    1000: { selfBonus: 188, uplineBonus: 0 },
    5000: { selfBonus: 288, uplineBonus: 0 },
    10000: { selfBonus: 488, uplineBonus: 0 },
    50000: { selfBonus: 2888, uplineBonus: 0 },
    100000: { selfBonus: 5008, uplineBonus: 0 },
  };
