...
const loginadminPage = async (req, res) => {
  return res.render("account/adminlogin.ejs");
};
const adminLoginPage = async (req, res) => {
  return res.render("admin/login.ejs");
};
const loginfinancePage = async (req, res) => {
  return res.render("account/financelogin.ejs");
};
const logindepositPage = async (req, res) => {
  return res.render("account/depositlogin.ejs");
};
const loginwithdrawPage = async (req, res) => {
  return res.render("account/withdrawlogin.ejs");
};
...
const login = async (req, res) => {
  ...
  try {
    ...
    // console.log("object", md5(accessToken), username)
    await connection.query( // 🟢 Use query() instead of execute()
...
const isdemoregister = async (req, res) => {
  try {
    const schema = Joi.object({
      username: Joi.string().length(10).required(),
      pwd: Joi.string().min(6).required(),
    ...
    let otp = utils.generateUniqueNumberCodeByDigit(6);
    let name_user = "Demo" + utils.generateUniqueNumberCodeByDigit(5);
    let code = utils.generateUniqueNumberCodeByDigit(5) + id_user;
    let bonus_money = process.env.BONUS_MONEY_ON_REGISTER;
    let real_money = process.env.REAL_MONEY_ON_REGISTER;
    ...
    const hashedPassword = await bcrypt.hash(pwd, saltRounds);
    const sql =
      "INSERT INTO users SET id_user = ?,phone = ?,name_user = ?,password = ?,plain_password = ?, money = ?,bonus_money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?,isdemo=?";
    await connection.execute(sql, [
      id_user,
      username,
      name_user,
...
const updateUsernameAPI = async (req, res) => {
  try {
    ...
    let auth = req.cookies.auth;
    let nickname = _.trim((req.body && req.body.nickname) || "");
...
const updateAvatarAPI = async (req, res) => {
  try {
    ...
    let auth = req.cookies.auth;
    let avatar = _.trim((req.body && req.body.avatar) || "");
