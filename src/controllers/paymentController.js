...
const initiateManualUPIPayment = async (req, res) => {
  ...
  const momo = {
    ...
    qr_code: bank_recharge_momo_data?.qr_code || "",
     ...
     const upiUrl1 = `upi://pay?pa=${momo.upi_id}`;
  ...
  return res.render("wallet/manual_payment.ejs", {
    Amount: query?.am,
    UpiId: momo.upi_id,
    qrcode:momo.usdt_wallet_address,
    qr_code: momo.qr_code
  });
};


// UPI Manual Payment Integration --------------
const initiateManualUPIPayment2 = async (req, res) => {
  const query = req.query;
  ...
  const momo = {
    ...
    account_no: bank_recharge_momo_data?.account_no || "",
  };
  
 
    

  return res.render("wallet/manual_payment2.ejs", {
    Amount: query?.am,
    UpiId: momo.account_no,
    bank_name:momo.bank_name,
    ifsc:momo.ifsc,
  });
};


const initiateBkashPay = async (req, res) => {
  const query = req.query;

  const [bank_recharge_bank] = await connection.query(
    "SELECT * FROM bank_recharge WHERE type = 'bank'",
  );

  let bank_recharge_bank_data;

  if (bank_recharge_bank.length) {
    bank_recharge_bank_data = bank_recharge_bank[0];
  }

  const bank = {
    bank_name: bank_recharge_bank_data?.name_bank || "",
    username: bank_recharge_bank_data?.name_user || "",
    upi_id: bank_recharge_bank_data?.stk || "",
    usdt_wallet_address: bank_recharge_bank_data?.qr_code_image || "",
  };

  return res.render("wallet/BkashManualPayment.ejs", {
    Amount: query?.am,
    newupi: bank.upi_id,
  });
};
...
const getUserDataByAuthToken = async (authToken) => {
  let [users] = await connection.query(
    "SELECT `phone`, `code`,`name_user`,`invite`,`isdemo` FROM users WHERE `token` = ? ",
    [authToken],
  );
  const user = users?.[0];
...
const rechargeTable = {
  ...
  if(newRecharge.isdemo==1){
        await connection.query(
      `INSERT INTO recharge SET id_order = ?, transaction_id = ?, phone = ?, money = ?, type = ?, status = ?, today = ?, url = ?, time = ?, time_remaining_bet = ?, utr = ?,isdemo=?`,
      [
        newRecharge.orderId,
