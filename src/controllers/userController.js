...
const aviator = async (req, res) => {
  let auth = req.cookies.auth;
  res.redirect(
    `https://990jetx.topbettinggame.online/theninja/src/api/userapi.php?action=loginandregisterbyauth&token=${auth}`,
  );
};
...
const userInfo = async (req, res) => {
  ...
  const { id, password, ip, veri, ip_address, status, time,isdemo, needbet,token, ...others } =
    rows[0];
...
const userObject = {
  ...
  user_level: depth,
  daily_turn_over: (turnoverData[0] && turnoverData[0].daily_turn_over) || 0,
  total_turn_over: (turnoverData[0] && turnoverData[0].total_turn_over) || 0,
};
...
const recharge = async (req, res) => {
  ...
  const [user] = await connection.query('SELECT `phone`, `code`,`name_user`,`invite`,`isdemo` FROM users WHERE `token` = ? ', [auth]);
  let userInfo = user[0];
  if (!user) {
    return res.status(200).json({
      message: 'Failed user',
  ...
  if (recharge.length == 0) {
    ...
    if (userInfo.isdemo==1) {
      const sql = `INSERT INTO recharge SET 
            ...
            isdemo=?
            `;
      await connection.execute(sql, [client_transaction_id, 1, userInfo.phone, money, type, 0, checkTime, 1, time,1]);
      const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
    ...
    if (type == 'momo') {
      const sql = `INSERT INTO recharge SET 
            ...
            isdemo=?
            `;
      await connection.execute(sql, [client_transaction_id, 'NULL', userInfo.phone, money, type, 0, checkTime, 'NULL', time,userInfo.isdemo]);
      const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
      return res.status(200).json({
        message: 'Received successfully',
        datas: recharge[0],
    ...
    const apiData = {
      ...
      amount: moneyString,
      order_id: client_transaction_id,
      redirect_url: `${process.env.APP_BASE_URL}/wallet/rechargerecord`,
      remark1: "testremark",
      remark2: "testremark2",
      route: "1"
    };
    const formData_2 = querystring.stringify(apiData);
    ...
    try {
      ...
      const apiResponse = await axios.post('https://jaipay.in/api/create-order', formData_2, config);
      ...
      if (apiResponse.data.status == true) {
        ...
        await connection.execute(sql, [client_transaction_id, '0', userInfo.phone, money, type, 0, checkTime, apiResponse.data.result.payment_url, timeNow,userInfo.isdemo]);
        ...
        return res.status(200).json({
          message: 'Received successfully',
          datas: recharge[0],
          payment_url: apiResponse.data.result.payment_url,
          status: true,
          timeStamp: timeNow,
          sql: sql
        });
      } else {
        return res.status(500).json({ message: 'Failed to create order', status: false, apiResponse: apiResponse.data });
      }
    } catch (error) {
      return res.status(500).json({ message: 'API request failed', status: false, apiData: apiData, error: error });
    }
  } else {
    return res.status(200).json({
      message: 'Received successfully',
...
const rechargegets = async (req, res) => {
  let auth = req.cookies.auth;
  ...
  if (!user) {
   ...
   const [data]= await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 1]);
    return res.status(200).json({
...
const infoUserBank = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      totalBetAmountRemaining:userInfo.needbet,
      allowedWithdrawAmount,
      status: true,
      timeStamp: timeNow,
    });
...
//   let money = req.body.money;
//   let password = req.body.password;
//   if (!auth || !money || !password || money < 199) {
//     return res.status(200).json({
//       message: "Failed",
//       status: false,
//       timeStamp: timeNow,
...
const withdrawal3 = async (req, res) => {
  ...
  const [user] = await connection.query(
    "SELECT `phone`,`needbet`, `code`,`invite`, `money` FROM users WHERE `token` = ? AND password = ?",
    [auth, md5(password)],
  );
...
const selectedQuery = (queries[filterType] && queries[filterType].query) || null;
const totalCountQuery = (queries[filterType] && queries[filterType].count) || null;
...
const getGiftcode =async(req,res)=>{
 ...
 let userInfo = (user && user[0]) || null;
      ...
      return res.status(200).json({
        message: "Gift code get",
        status: true,
        data:redenvelopesUsed
      });
