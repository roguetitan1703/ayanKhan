...
const createWithdrawalRequest = async (req, res) => {
  ...
  try {
    ...
    if(user.isdemo===1){
      return res.status(200).json({
        message: "You can't withdrawals with demo account",
        status: true,
        timeStamp: timeNow,
      });
    }
...
const approveOrDenyWithdrawalRequest = async (req, res) => {
  ...
  try {
    ...
    if (status == WITHDRAWAL_STATUS_MAP.ACCEPT) {
...
const getUserDataByAuthToken = async (authToken) => {
  let [users] = await connection.query(
    "SELECT `phone`, `code`,`name_user`,`invite`,`money`,`isdemo` FROM users WHERE `token` = ? ",
    [authToken],
  );
  const user = users?.[0];
