...
const allRecharge = async (req, res) => {
  return res.render("daily/allRecharge.ejs");
};

const allWithdraw = async (req, res) => {
  return res.render("daily/allWithdraw.ejs");
};
// deposit manager
const depositmanagerPage = async (req, res) => {
  return res.render("depositmanager/statistical.ejs");
};
const withdrawmanagerPage = async (req, res) => {
  return res.render("withdrawmanager/statistical.ejs");
};
const depositmanagerlistRecharge = async (req, res) => {
  return res.render("depositmanager/listRecharge.ejs");
};
const withdrawmanagerlistWithdraw = async (req, res) => {
  return res.render("withdrawmanager/listWithdraw.ejs");
};
const depositmanagerallRecharge = async (req, res) => {
  return res.render("depositmanager/allRecharge.ejs");
};

const withdrawmanagerallWithdraw = async (req, res) => {
  return res.render("withdrawmanager/allWithdraw.ejs");
};
...
const infoCtv = async (req, res) => {
  ...
  for (let i = 0; i < list_mem.length; i++) {
    ...
    const [recharge] = await connection.query(
      "SELECT SUM(money) as money FROM recharge WHERE phone = ? AND status = 1 AND isdemo=0",
      [phone],
    );
    const [withdraw] = await connection.query(
      "SELECT SUM(money) as money FROM withdraw WHERE phone = ? AND status = 1  AND isdemo=0",
      [phone],
    );
    if (recharge[0].money) {
...
const dailyController = {
  ...
  depositmanagerPage,
  withdrawmanagerPage,
  depositmanagerallRecharge,
  withdrawmanagerallWithdraw,
  depositmanagerlistRecharge,
  withdrawmanagerlistWithdraw,
};
